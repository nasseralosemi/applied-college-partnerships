...
export const TabsNav: React.FC<TabsNavProps> = ({
  ...
  return (
    <nav aria-label="شريط التبويبات الرئيسي" className="no-print bg-slate-50/95 backdrop-blur-md border-b border-slate-200/90 shadow-xs sticky top-[73px] z-30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center space-x-reverse space-x-2 sm:space-x-4 overflow-x-auto py-2.5 no-scrollbar">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-xs sm:text-sm whitespace-nowrap transition-all duration-200 cursor-pointer border ${
                  isActive
              ...
              >
                ...
                {typeof tab.count === 'number' && (
                  <span
                    className={`mr-1 px-2 py-0.5 rounded-full text-[11px] font-bold ${
                      isActive
